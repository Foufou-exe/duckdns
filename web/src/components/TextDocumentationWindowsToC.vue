<template>
    <h1 class="text-4xl font-extrabold mb-5">Windows - C# Visual Studio 2022 <font-awesome-icon :icon="['fab', 'windows']" class="text-primary"/></h1>
    <p class="font-thin mb-5">
        This is an Open Source tray based service that was created by XWolfOverride.
        <div class="p-5 bg-neutral rounded text-white mt-2 mb-2">
            <h3 class="font-semibold mb-1">DuckDNS - XWolfOverride <div class="badge badge-success badge-outline">Verified</div></h3>
            <font-awesome-icon :icon="['fab', 'github']" /> <a href="https://github.com/XWolfOverride/DuckDNS" class="link">https://github.com/XWolfOverride/DuckDNS</a>
        </div>
    </p>
</template>

<script setup>
//
</script>